...
export default {
  ...
  data() {
    return {
      ...
      backPath: '',
      token: '',
      path: '/search-idcard-family',
      goPath: '',
      content: '',
      isloading: true,
      isActivate: false,
      lang: this.$t('searchIdcardFamily'),
  ...
  mounted() {
    const vm = this
    firebase.analytics().logEvent('search-idcard-family-page')
  ...
  methods: {
    ...
    validate() {
      const vm = this
      firebase.analytics().logEvent('search-idcard-family-button')
      vm.isloading = true
      vm.clearValidate()
      if (vm.checkValidate()) {
    ...
    async checkID() {
      ...
      try {
        ...
        await vm.$axios
          .get(
            `/policy/list/${vm.idCard}?insured_type=family&lang=${vm.locale}`
          )
          .then((response) => {
    ...
    summitPopup() {
      ...
      if (vm.isSuccess) {
        ...
        if (vm.locale === 'th') {
          vm.setPath('/')
          vm.$router.push(`/family-coverages/${vm.idCard}`)
        } else if (vm.locale === 'en') {
          vm.setPath('/en')
          vm.$router.push(`/en/family-coverages/${vm.idCard}`)
        }
