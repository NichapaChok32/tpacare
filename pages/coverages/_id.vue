  ...
  >
    ...
    <v-container v-if="!isLoading && !isShowPictures" class="Coverage-detail">
      <v-row
        ...
        class="coverage-card"
        :class="{
          hideButton: currentInsurance.inapp,
          fullPadding:
      ...
      >
        ...
        <div v-if="!isAddCard && isOpenDetail" class="div-control">
          <v-row
            v-if="
...
export default {
  ...
  data() {
    return {
      ...
      backPath: '',
      fields_list: [
        { key: 'CovNo', label: this.$t('my-coverages-id').detailHeader.order },
        {
          key: 'CovDesc',
          label: this.$t('my-coverages-id').detailHeader.benefit,
        },
        {
          key: 'CovLimit',
          label: this.$t('my-coverages-id').detailHeader.coverage,
        },
        {
          key: 'CovUtilized',
          label: this.$t('my-coverages-id').detailHeader.used,
        },
      ],
      fields_ext: [
        { key: 'CovNo', label: '' },
      ...
      fields_coverage: [
        {
          key: 'cov_index',
          label: this.$t('my-coverages-id').detailHeader.order,
        },
        {
          key: 'cov_desc',
          label: this.$t('my-coverages-id').detailHeader.benefit,
  ...
  methods: {
    ...
    openDetail() {
      ...
      if (vm.locale === 'th') {
        vm.setPath(`/coverages/${vm.id}`)
        vm.$router.push('/view-card-detail')
      } else if (vm.locale === 'en') {
        vm.setPath(`/en/coverages/${vm.id}`)
      ...
      for (let i = 0; i < vm.insurance.length; i++) {
        if (vm.mobileList.length === 0) {
          if (
            ...
            vm.insurance[i].telNo !== null &&
            vm.insurance[i].telNo !== '' &&
            vm.insurance[i].telNo !== ' '
          ) {
    ...
    async getInsurance() {
      ...
      try {
        ...
        await vm.$axios
          .get(`/policy/list/${vm.id}?insured_type=member&lang=${vm.locale}`)
          .then((response) => {
            ...
            firebase.analytics().logEvent('coverages-by-id')
          })
      } catch (e) {
    ...
    async getDetail() {
      ...
      try {
        ...
        firebase.analytics().logEvent('coverages-details')
      } catch (e) {
        // eslint-disable-next-line no-console
        console.log(e)
        vm.isLoadingDetail = false
    ...
    goAddCard() {
      const vm = this
      firebase.analytics().logEvent('addcard-button')
