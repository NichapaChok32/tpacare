...
export default {
  ...
  data() {
    return {
      ...
      backPath: '',
      fields_list: [
        { key: 'CovNo', label: this.$t('family-coverages').detailHeader.order },
        {
          key: 'CovDesc',
          label: this.$t('family-coverages').detailHeader.benefit,
        },
        {
          key: 'CovLimit',
          label: this.$t('family-coverages').detailHeader.coverage,
        },
        {
          key: 'CovUtilized',
          label: this.$t('family-coverages').detailHeader.used,
        },
      ],
      fields_ext: [
        { key: 'CovNo', label: '' },
      ...
      fields_coverage: [
        {
          key: 'cov_index',
          label: this.$t('family-coverages').detailHeader.order,
        },
        {
          key: 'cov_desc',
          label: this.$t('family-coverages').detailHeader.benefit,
  ...
  methods: {
    ...
    async getInsurance() {
      ...
      try {
        ...
        await vm.$axios
          .get(`/policy/list?insured_type=family&lang=${vm.locale}`)
          .then((response) => {
        ...
        firebase.analytics().logEvent('family-coverages-page')
      } catch (e) {
    ...
    useCard() {
      const vm = this
      firebase.analytics().logEvent('family-uaed-card-button')
      vm.setInsurance(vm.currentInsurance)
      if (vm.locale === 'th') {
        vm.setPath('/family-coverages')
        vm.$router.push('/hospital-list')
      } else if (vm.locale === 'en') {
        vm.setPath('/en/family-coverages')
    ...
    eClaim() {
      ...
      firebase.analytics().logEvent('family-eclaim-button')
      if (vm.locale === 'th') {
        vm.setPath('/family-coverages')
    ...
    goAddCard() {
      const vm = this
      firebase.analytics().logEvent('family-addcard-button')
    ...
    goSearchIdCard() {
      const vm = this
      firebase.analytics().logEvent('family-search-idcard-button')
      if (vm.locale === 'th') {
        vm.setPath('/family-coverages')
