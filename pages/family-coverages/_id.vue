<template>
  ...
  >
    ...
    <v-container v-if="!isLoading && !isShowPictures" class="Coverage-detail">
      <v-row v-if="!isNotInsurance" class="coverage-card">
        <v-carousel
          id="carouselFamilyCard"
          v-model="cardIndex"
          :show-arrows="false"
          :class="{
        ...
        >
          <v-carousel-item v-for="item in insurance" :key="item.id">
            <TPACard :card="item" :openDetail="isOpenDetail" />
          </v-carousel-item>
          ...
          <ol
            ...
            class="carousel-family-indicators"
            :class="{
        ...
        <div v-if="!isAddCard && isActivate" class="div-control">
          <v-row
            v-if="
      ...
      >
        <div
          v-if="!isAddCard && !isOpenDetail && !isActivate"
...
export default {
  ...
  data() {
    return {
      ...
      fields_list: [
        {
          key: 'CovNo',
          label: this.$t('family-coverages-id').detailHeader.order,
        },
        {
          key: 'CovDesc',
          label: this.$t('family-coverages-id').detailHeader.benefit,
        },
        {
          key: 'CovLimit',
          label: this.$t('family-coverages-id').detailHeader.coverage,
        },
        {
          key: 'CovUtilized',
          label: this.$t('family-coverages-id').detailHeader.used,
        },
      ],
      fields_ext: [
        { key: 'CovNo', label: '' },
      ...
      isShowAdd: false,
      isOpenDetail: false,
      lang: this.$t('family-coverages-id'),
  ...
  mounted() {
    ...
    vm.getInsurance()
    if (vm.$auth.$storage.getLocalStorage('ViewCardAuthenication')) {
      vm.authenicationCard()
    }
  },
  methods: {
    ...mapMutations({
      ...
      resetMobileList: 'resetMobileList',
    ...
    openDetail() {
      const vm = this
      vm.setMobileList(vm.mobileList)
      if (vm.locale === 'th') {
        vm.setPath(`/family-coverages/${vm.id}`)
        vm.$router.push('/view-card-detail')
      } else if (vm.locale === 'en') {
        vm.setPath(`/en/family-coverages/${vm.id}`)
        vm.$router.push('/en/view-card-detail')
      }
    },
    getMobileList() {
      const vm = this
      for (let i = 0; i < vm.insurance.length; i++) {
        if (vm.mobileList.length === 0) {
          if (vm.insurance[i].telNo !== undefined) {
            vm.mobileList.push(vm.insurance[i].telNo)
          }
        } else {
          for (let j = 0; j < vm.mobileList.length; j++) {
            if (vm.insurance[i].telNo !== vm.insurance[i].mobile) {
              if (
                vm.insurance[i].telNo !== undefined &&
                vm.insurance[i].telNo !== vm.mobileList[j]
              ) {
                vm.mobileList.push(vm.insurance[i].telNo)
              }
            }
    ...
    async getInsurance() {
      ...
      try {
        ...
        await vm.$axios
          .get(`/policy/list/${vm.id}?insured_type=family&lang=${vm.locale}`)
          .then((response) => {
            ...
            if (res.status === 'ok' && res.data.length !== 0) {
              for (let i = 0; i < res.data.length; i++) {
                vm.insurance[i] = {
                  ...
                  isUseCard: vm.checkStartDate(res.data[i].EffFrom),
                }
              }
        ...
        firebase.analytics().logEvent('family-coverages-id-page')
      } catch (e) {
    ...
    useCard() {
      const vm = this
      firebase.analytics().logEvent('family-coverages-used-card-button')
    ...
    eClaim() {
      ...
      firebase.analytics().logEvent('family-coverages-eclaim-button')
      if (vm.locale === 'th') {
        vm.setPath(`/family-coverages/${vm.id}`)
    ...
    goAddCard() {
      const vm = this
      firebase.analytics().logEvent('family-coverages-addcard-button')
      vm.setIsAddCard(true)
      if (vm.locale === 'th') {
        vm.setPath(`/family-coverages/${vm.id}`)
        vm.$router.push('/add-card-family')
      } else if (vm.locale === 'en') {
        vm.setPath(`/en/family-coverages/${vm.id}`)
        vm.$router.push('/en/add-card-family')
      }
    },
    goToZoom(value) {
      const vm = this
    ...
    goSearchIdCard() {
      ...
      if (vm.locale === 'th') {
        vm.setPath(`/family-coverages/${vm.id}`)
        vm.$router.push('/search-idcard-family')
      } else if (vm.locale === 'en') {
        vm.setPath(`/family-coverages/${vm.id}`)
        vm.$router.push('/en/search-idcard-family')
      }
    },
  },
}
...
<style>
@import url('../../assets/css/family-coverages.css');
</style>
