...
export default {
  ...
  mounted() {
    ...
    if (vm.isActivate) {
      ...
      if (vm.locale === 'th') {
        vm.backPath = `/coverages/${vm.idCard}`
      } else if (vm.locale === 'en') {
        vm.backPath = `/en/coverages/${vm.idCard}`
      }
    }
    vm.getCardDetail()
  },
  methods: {
        ...
        await vm.$axios
          ...
          .then((response) => {
            ...
            if (res.status === 'ok') {
              ...
              vm.isloading = false
              firebase.analytics().logEvent('edit-card-page')
            }
          })
      } catch (e) {
    ...
    goEditCoverage() {
      const vm = this
      firebase.analytics().logEvent('edit-benefit-button')
      if (vm.locale === 'th') {
        vm.setPath(`/edit-card/${vm.id}`)
        vm.setCoverage(vm.card.coverages)
        vm.$router.push('/edit-coverage')
      } else if (vm.locale === 'en') {
        vm.setPath(`/en/edit-card/${vm.id}`)
        vm.setCoverage(vm.card.coverages)
        vm.$router.push('/en/edit-coverage')
      }
    },
    goToInsurer() {
      const vm = this
      firebase.analytics().logEvent('edit-insurer-button')
      if (vm.locale === 'th') {
        vm.setPath(`/edit-card/${vm.id}`)
    ...
    editCoverage() {
      const vm = this
      firebase.analytics().logEvent('edit-card-button')
      vm.isloading = true
      vm.clearValidate()
      if (vm.validate()) {
        event.preventDefault()
    ...
    async deletePolicy() {
      ...
      try {
        ...
        firebase.analytics().logEvent('delete-card-button')
      } catch (e) {
        // eslint-disable-next-line no-console
        console.log(e)
        vm.isloading = false
